...
if(isset($_POST["category"])){
	...
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			...
			echo "
					<li><a href='#' class='Categorias' cid='$cid'>$cat_name</a></li>
			";
		}
		echo "</div>";
	}
		...
		if($count > 0){
			echo "
				<div class='alert alert-warning'>
						...
						<b>Este produto ja foi colocado no carrinho de compras</b>
				</div>
			";//not in video
		} else {
			...
			if (mysqli_num_rows($query) > 0) {
				echo "
					<div class='alert alert-warning'>
							<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
							<b>Produto já foi colocado no carrinho de compras!</b>
					</div>";
					exit();
			}
			$sql = "INSERT INTO `cart`
	...
	if (isset($_POST["checkOutDetails"])) {
		if (mysqli_num_rows($query) > 0) {
			...
			echo "<form method='post' action='login_form.php'>";
				...
				if (!isset($_SESSION["uid"])) {
					echo '<input type="submit" style="float:right;" name="login_user_with_product" class="btn btn-info btn-lg" value="Pronto pra checar" >
							</form>';
							...
							echo   
								'<input type="hidden" name="return" value="http://localhost/project1/payment_success.php"/>
									...
									<input style="float:right;margin-right:80px;" type="image" name="submit"
										src="https://www.paypalobjects.com/webstatic/en_US/i/btn/png/blue-rect-paypalcheckout-60px.png" alt="Verificação com Paypal"
